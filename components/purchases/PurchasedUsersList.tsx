...
import { PurchasedUserCard } from "./PurchasedUserCard";
...
export function PurchasedUsersList({
  coursePurchases,
  bundlePurchases,
  courses,
  bundles,
  ...
  onExportBundleCSV,
  showBundleTab = true,
}: PurchasedUsersListProps) {
  const [searchQuery, setSearchQuery] = useState("");
  const [activeTab, setActiveTab] = useState<"course" | "bundle">("course");

  const filteredCoursePurchases = useMemo(() => {
    if (!searchQuery.trim()) return coursePurchases;
    const query = searchQuery.toLowerCase();
    return coursePurchases.filter(
      (user) =>
  ...
  const filteredBundlePurchases = useMemo(() => {
    if (!searchQuery.trim()) return bundlePurchases;
    const query = searchQuery.toLowerCase();
    return bundlePurchases.filter(
      (user) =>
        user.name?.toLowerCase().includes(query) ||
        user.email?.toLowerCase().includes(query) ||
        user.phone?.includes(query)
    );
  }, [bundlePurchases, searchQuery]);

  const totalCourseAmount = useMemo(
    () => coursePurchases.reduce((sum, user) => sum + (user.amount || 0), 0),
    [coursePurchases]
  );

  const totalBundleAmount = useMemo(
    () => bundlePurchases.reduce((sum, user) => sum + (user.amount || 0), 0),
    [bundlePurchases]
  );

  const isCourseTab = activeTab === "course";
  const activePurchases = isCourseTab
    ? filteredCoursePurchases
    : filteredBundlePurchases;
  const activeAmount = isCourseTab ? totalCourseAmount : totalBundleAmount;
  ...
  const emptyTitle = isCourseTab
    ? "No course buyers found"
    : "No bundle buyers found";
      ...
      <CardContent className="p-3">
        ...
        >
          <div className="grid gap-2 xl:grid-cols-[auto_minmax(220px,1fr)_minmax(260px,1.2fr)_auto_auto] xl:items-center">
            <TabsList
              className={`grid h-9 gap-1 rounded-xl bg-background/60 p-1 ${
                showBundleTab ? "grid-cols-2" : "grid-cols-1"
              }`}
            >
              <TabsTrigger
                value="course"
            ...
            <div className="whitespace-nowrap rounded-xl border border-border/60 bg-background/45 px-3 py-2 text-xs text-muted-foreground">
              <span className="font-semibold text-foreground">
                {activePurchases.length.toLocaleString()}
              </span>{" "}
              users ·{" "}
              <span className="font-semibold text-foreground">
          ...
          <TabsContent value="course" className="mt-0 space-y-1.5">
            {activeLoading ? (
              <PurchaseSkeleton />
            ) : filteredCoursePurchases.length === 0 ? (
              <EmptyState title={emptyTitle} />
            ) : (
              filteredCoursePurchases.map((user, index) => (
                <PurchasedUserCard
          ...
          {showBundleTab && (
            <TabsContent value="bundle" className="mt-0 space-y-1.5">
              {activeLoading ? (
                <PurchaseSkeleton />
              ) : filteredBundlePurchases.length === 0 ? (
                <EmptyState title={emptyTitle} />
              ) : (
                filteredBundlePurchases.map((user, index) => (
                  <PurchasedUserCard
                    key={user.user_id || index}
                    user={user}
                    type="bundle"
