...
- **POST** `/managerial/login-unified` or **POST** `/managerial/login`  
...
- `login` can be **email** or **phone** (unified flow normalizes it).
...
| `type`         | number   | Account type: `1` = admin, `2` = moderator, `3` = regular, `4` = campusAmbassador. Admin APIs expect `1` or `2`. |
| `login`        | string   | Login identifier (email or phone). |
| `loginType`    | string   | e.g. `"email"` or `"phone"`. |
...
- **Visibility:** Use `permissions` to show/hide admin dashboard tabs or features. Example: show “Users” tab only if `permissions.includes('user.manage.all')`. See **docs/ADMIN_DASHBOARD_TABS_BY_PERMISSIONS.md** for a full tab–permission mapping.
...
| campusAmbassador | 4 | Campus ambassador |
...
| Task | Action |
|------|--------|
| Log in | **POST** `/managerial/login-unified` (or `/managerial/login`) with `{ login, password }`. |
...
| Show/hide UI | Use `user.permissions` (or decoded token `permissions`) with a mapping (see ADMIN_DASHBOARD_TABS_BY_PERMISSIONS.md). |
