...
**Description:** Returns the full list of permission strings the backend recognizes. Use this when building the **Create role** or **Edit role** form so the frontend always shows the same permissions the backend accepts (no need to maintain a separate list on the frontend).
...
- `success`: `true`
- `data`: object with two shapes of the same permission list:
  - **`data.all`** — Array of every permission string the backend accepts (e.g. `["user.manage.all", "course.manage.all", "role.manage.all", ...]`). Same permissions as in `by_resource`, but in one flat list. Use this when you want a single list (e.g. one set of checkboxes, or a searchable list). The frontend should only send permission strings that appear in this array when creating or updating a role.
  - **`data.by_resource`** — Object keyed by resource name in UPPERCASE (e.g. `USER`, `COURSE`, `ROLE`, `PAYMENT`, `BUNDLE`). Each key’s value is an array of permission strings for that resource only (e.g. `by_resource.USER` might be `["user.create.all", "user.read.all", "user.manage.all", ...]`). Use this when you want to show permissions in sections (e.g. “User”, “Course”, “Role”) so the admin can pick by category. When submitting the form, collect all selected strings from every section into one array and send as `permissions`.
- `count`: Number; total number of permissions. Always equals `data.all.length`. Use for “X of N selected” or to confirm the list loaded.
...
- **400** — Validation or business rule failure:
  ...
  - System role (e.g. `admin`, `moderator`, `student`, `teacher`, `ambassador`): `error`: `DELETE_FAILED`, `message`: "Cannot delete system roles".
